	...
	protected void onHandleIntent(Intent intent) {
		...
		try {
			...
			Cursor non_delivered = getContentResolver().query(urimsg0, new String[]{}
					, ChatMessageContract.IS_DELIVERED + " = ? AND " + ContactContract.IS_GROUP + " = ?"
					, new String[]{ "0", "0" }, "ASC" );
			...
			if (non_delivered.moveToFirst()) {
				do{
					...
					deliveryack.put("receiver_id", non_delivered.getInt(non_delivered.getColumnIndex(ChatMessageContract.CREATOR_ID)));
